...
const DIFFICULTY_CONFIG = {
  easy: { speed: 5, spawnInterval: 1100, gravity: 1.8, jumpStrength: 24, maxHeight: 35 },
  normal: { speed: 7, spawnInterval: 900, gravity: 2.1, jumpStrength: 26, maxHeight: 40 },
  hard: { speed: 10, spawnInterval: 700, gravity: 2.3, jumpStrength: 28, maxHeight: 45 },
};
...
interface GameDashEnhancedProps {
  difficulty: Difficulty;
  onExit: () => void;
}

export const GameDashEnhanced: React.FC<GameDashEnhancedProps> = ({ difficulty, onExit }) => {
  const { profile, addTokens, addXP, updateGeometryScore } = usePlayer();
  ...
  const handleJump = () => {
    ...
    playRetroSound('JUMP');
    setVelocityY(-config.jumpStrength);
    setJumpCount((c) => c + 1);
  };
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (isRunning) {
        setVelocityY((vy) => vy + config.gravity * delta);
        setPlayerY((y) => {
        ...
        setObstacles((prev) => {
          const updated = prev
            .map((ob) => ({ ...ob, x: ob.x - config.speed * delta * 3 }))
          ...
          if (time - lastSpawnRef.current > config.spawnInterval) {
            ...
            const height = 25 + Math.random() * (config.maxHeight - 25);
  ...
  useEffect(() => {
    ...
    for (const ob of obstacles) {
      ...
      if (overlapX && overlapY) {
        ...
        updateGeometryScore(score);
        const baseTokens = difficulty === 'easy' ? 10 : difficulty === 'normal' ? 25 : 50;
        const baseXP = difficulty === 'easy' ? 20 : difficulty === 'normal' ? 50 : 100;
        const tokenReward = Math.floor(baseTokens + score / 100);
        const xpReward = Math.floor(baseXP + score / 50);
  ...
  }, [obstacles, playerY, isRunning, score, difficulty, addTokens, addXP, updateGeometryScore]);
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center select-none">
      <div className="mb-4 flex items-center justify-between w-full max-w-xl px-4 text-[10px] md:text-xs font-['Share_Tech_Mono'] tracking-[0.35em] text-cyan-200">
        <span>WILEX // GEOMETRY TRIAL [{difficulty.toUpperCase()}]</span>
            ...
            {rewardShown && (
              ...
              >
                <div className="text-[10px] md:text-xs font-['Share_Tech_Mono'] text-emerald-300 mb-1">
                  +{Math.floor(10 + score / 100)} TOKENS
                </div>
                <div className="text-[10px] md:text-xs font-['Share_Tech_Mono'] text-cyan-300">
                  +{Math.floor(20 + score / 50)} XP
                </div>
              </motion.div>
            )}
            <div className={`text-[10px] md:text-xs font-['Share_Tech_Mono'] ${difficultyColor[difficulty]} mb-4`}>
